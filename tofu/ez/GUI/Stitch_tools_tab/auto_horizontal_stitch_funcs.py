...
class AutoHorizontalStitchFunctions:
    ...
    def stitch_images(self):
        for i, (ctdir, ax) in enumerate(self.ct_axis_dict.items()):
            ...
            cra = float(self.greatest_axis_value) - float(ax)
    ...
    def compute_centers(self):
        ...
        fdt_names = {
            ...
            'common_flats2': "",
            'use_common_flats2': False,
            'use_shared_flatsdarks': self.parameters['common_flats_darks']
        } #TODO ask if I should add common_flats2 box in GUI
        
        W, self.lvl0 = get_CTdirs_list(self.parameters['input_dir'], fdt_names)
        for i, ctset in enumerate(W):          
            search_slice = int(self.parameters['search_slice'])
            nviews, wh, multipage = get_dims(os.path.join(ctset[0], fdt_names['tomo']))
